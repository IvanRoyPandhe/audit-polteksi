<x-dashboard-layout>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Audit</h1>
        <p class="text-gray-600 mt-1">Buat audit baru</p>
    </div>
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('audit.store') }}">
                ...
                <div class="mb-6">
                    ...
                    <select name="validasi_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        @foreach($validasis as $validasi)
                            <option value="{{ $validasi->validasi_id }}" {{ old('validasi_id') == $validasi->validasi_id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="date" name="tanggal_audit" value="{{ old('tanggal_audit', date('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                    @error('tanggal_audit')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="status_audit" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="Direncanakan" {{ old('status_audit') == 'Direncanakan' ? 'selected' : '' }}>Direncanakan</option>
                        <option value="Berlangsung" {{ old('status_audit') == 'Berlangsung' ? 'selected' : '' }}>Berlangsung</option>
                        <option value="Selesai" {{ old('status_audit') == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                    </select>
                    @error('status_audit')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="number" step="0.01" name="skor_total" value="{{ old('skor_total') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('skor_total')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
