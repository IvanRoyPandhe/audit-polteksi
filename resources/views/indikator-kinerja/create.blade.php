<x-dashboard-layout>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Indikator Kinerja</h1>
        <p class="text-gray-600 mt-1">Buat indikator kinerja baru</p>
    </div>

    <div class="bg-white rounded-lg shadow">
        <div class="p-6">
            <form method="POST" action="{{ route('indikator-kinerja.store') }}">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="standar_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        @foreach($standars as $standar)
                            <option value="{{ $standar->standar_id }}" {{ old('standar_id') == $standar->standar_id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nama_indikator" value="{{ old('nama_indikator') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" step="0.01" name="target" value="{{ old('target') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                    @error('target')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
