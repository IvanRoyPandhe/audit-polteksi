<x-dashboard-layout>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Temuan Audit</h1>
        <p class="text-gray-600 mt-1">Catat temuan hasil audit</p>
    </div>

    <div class="bg-white rounded-lg shadow">
        <div class="p-6">
            <form method="POST" action="{{ route('audit-temuan.store') }}">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="kategori_temuan" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="Non-Conformity" {{ old('kategori_temuan') == 'Non-Conformity' ? 'selected' : '' }}>Non-Conformity</option>
                        <option value="Observation" {{ old('kategori_temuan') == 'Observation' ? 'selected' : '' }}>Observation</option>
                        <option value="Opportunity" {{ old('kategori_temuan') == 'Opportunity' ? 'selected' : '' }}>Opportunity</option>
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="deskripsi_temuan" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>{{ old('deskripsi_temuan') }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <select name="severity" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="Kritis" {{ old('severity') == 'Kritis' ? 'selected' : '' }}>Kritis</option>
                    </select>
                    @error('severity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea name="rekomendasi" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('rekomendasi') }}</textarea>
                    @error('rekomendasi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
