<x-dashboard-layout>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Validasi</h1>
        <p class="text-gray-600 mt-1">Validasi data kinerja</p>
    </div>

    <div class="bg-white rounded-lg shadow">
        <div class="p-6">
            <form method="POST" action="{{ route('validasi.store') }}">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="kinerja_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        @foreach($dataKinerjas as $kinerja)
                            <option value="{{ $kinerja->kinerja_id }}" {{ old('kinerja_id') == $kinerja->kinerja_id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="date" name="tanggal_validasi" value="{{ old('tanggal_validasi', date('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div class="mb-6">
                    ...
                    <select name="status_validasi" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="Revisi" {{ old('status_validasi') == 'Revisi' ? 'selected' : '' }}>Revisi</option>
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="catatan" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
